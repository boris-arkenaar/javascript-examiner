\section{Fundamentals of providing feedback} This article contains the outcome 
of a limited study towards 
elucidating the fundamental aspects of providing \gls{feedback} on \gls{js-code}. 
...
while studying the course \gls{wac}. This research is one of 
the artefacts to realize some context and generate content, to extract 
requirements and recommendations for the remainder of the main \gls{project},
from the perspective of providing feedback.  
...
available) existing \gls{olp}s to learn JavaScript, in enquiring the
presence of didactics as proposed in academic writings and discovering some 
characteristic features. In addition, a \gls{tutor} with experience in examining
...
\citep{alomari2009investigating}, some related articles are consulted.
...
\citep{guri-rosenblit2005distance}
...
take place from tutor to \gls{student}, but also among students themselves and even
...
possible to provide feedback on the exercises and the platform itself, 
in a way the student experiences some kind of dialogue. For example, the 
student should be able to make suggestions to improve an exercise, or utter 
their difficulties while using the tool. The other
...
of \citep{shepherd2011graduate}, namely the demand of well written
(full sentences) 
...
This project  \citep{obrien2014java} has an interesting approach, for using a 
...
challenged to 
add test cases to this suite. To enforce the students to write these test cases,
the feedback functionality is only enabled if the student has already submitted
one or more test cases. This seems to be a little too much, as it undermines the
primary goal, and the skill of writing such test cases requires knowledge of the
language in the first place. Still, the idea of rewarding the student for a
contribution like adding a test case is something to keep in mind, as writing
test cases is an important aspect in programming, and part of the course
as well. To avoid a 
...
\gls{maintainability} and developmental perspective these might be in favour 
...
review the platforms. Based on the research in the previous chapter, the 
following characteristics and features will be reviewed:
\begin{itemize}
  \item Platform characteristics (web-based, command shell, client, \gls{user} interface)
  \item Exercises
    \begin{itemize}
  ...
  \item Feedback
    \begin{itemize}
	  ...
	  \item Extent (i.e. on \gls{syntax}, on \gls{functionality}, on \gls{semantics})
...
\gls{code}, a quite small area with instructions, an area to show output and some 
...
(covered in this project within the Domain research by Boris Arkenaar)
like modularity, design patterns, code elegance or 
